...
from land_cover_classification_unet.predict import LandcoverSegmentationClassifier
...
def make_frontend(
    ...
    examples_dir = Path("data") / "test_data"
    example_fnames = [f for f in os.listdir(examples_dir) if f.endswith(".png")]
...
class PredictorBackend:
    ...
    def __init__(self, url=None):
        if url is not None:
            ...
            model = LandcoverSegmentationClassifier(DEFAULT_MODEL)
    ...
    def _predict_from_endpoint(self, image, scale):
        ...
        """
        encoded_image = general.encode_b64_image(image)
